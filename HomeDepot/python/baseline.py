...
def str_norvig_correct(s): 
    if isinstance(s, str):
        s = (" ").join([norvig_correct(z) for z in s.split(" ")])
        return s
    else:
        return "null"
...
#print('start norvig correction')
#runfile('norvig_correct.py')
#df_all['search_term'] = df_all['search_term'].map(lambda x:str_norvig_correct(x))
#print('end norvig correction')
#
...
#df_all.to_csv('../input/df_all_spell_check_norvig.csv')


#Read process data frame
df_all = pd.read_csv('../input/df_all_spell_check.csv', encoding="ISO-8859-1", index_col=0)
#df_all = pd.read_csv('../input/df_all_spell_check_norvig.csv', encoding="ISO-8859-1", index_col=0)
...
#                        ('cnt1', pipeline.Pipeline([('s1', cust_txt_col(key='search_term')), ('tfidf1', countVect), ('tsvd1', tsvd)])),
#                        ('cnt2', pipeline.Pipeline([('s2', cust_txt_col(key='product_title')), ('tfidf2', countVect), ('tsvd2', tsvd)])),
#                        ('cnt3', pipeline.Pipeline([('s3', cust_txt_col(key='product_description')), ('tfidf3', countVect), ('tsvd3', tsvd)])),
#                        ('cnt4', pipeline.Pipeline([('s4', cust_txt_col(key='brand')), ('tfidf4', countVect), ('tsvd4', tsvd)]))
                        ],
                    transformer_weights = {
                        'cst': 1.0,
                        'txt1': 0.5,
